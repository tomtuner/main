...
abstract class Controller {
	...
	public function __construct() {
		if(CONTROLLER == 'index' && (ACTION == 'register' || ACTION == 'process_register')) {
			$this->account = new User();
		} else {
			try {
				//$this->account = new User(phpCAS::getUser());
				if($_SERVER['REMOTE_ADDR'] != '129.21.199.41') {
					$this->account = new User(phpCAS::getUser());
				} else {
					$this->account = new User('tecccl');
				}
	...
	public function pageTitle($page_title) {
		return 'Event Registration &raquo; '. implode(' &raquo; ', $page_title);
	}
